...
| What should be built next?        | The Critical Action Plan implementation queue is complete. Collect live usage/backfill/provider evidence before paid-limit enforcement, then select only evidence-gated work from the later queue.           |
...
| Quality gates                | Enforced test pyramid, 1,005 Jest assertions at this baseline, production build, Prisma/privacy/SEO checks, fresh migration and capacity runs, core-loop Playwright, WCAG A/AA axe audits, keyboard/focus checks, and retained failure evidence. | `config/test-pyramid.json`, `.github/workflows/test.yml`, `.github/workflows/e2e.yml`, `.github/workflows/load-testing.yml` |
| Usage and overhead           | Canonical per-business plan attribution, current usage, media-byte ledger/backfill, warning thresholds, stored planning rates, and a platform-admin usage dashboard/API.                                                                         | `src/lib/usage`, `src/app/admin/operations`, `src/app/api/admin/usage`, `docs/admin/USAGE_AND_OVERHEAD.md`                  |
...
| Plans and entitlements   | Free (£0), Professional (£8), and Business (£20) allowances now share canonical usage accounting; booking deposits have an explicit feature entitlement.  | Limits remain observational until live media backfill, provider billing evidence, downgrade/grace behaviour, and subscription state are reviewed. Subscription billing is not the current plan source of truth. |
| Analytics                | Consented Google Analytics hooks, first-sellable-loop progress, dashboard metrics, Web Vitals, and per-business usage/estimated overhead reporting exist. | CDN delivery/transformations, provider-reported or invoiced costs, referral attribution, and broader commercial analytics remain unavailable or deferred.                                                       |
...
1. **Paid-limit evidence:** run the historical media backfill, validate production provider billing
   alerts/rates, and observe real usage before enabling enforcement or making cost guarantees.
...
- Plan usage and thresholds are measured but not enforced. Historical media requires the documented
  Cloudinary backfill, and delivery/transformations remain unavailable until provider snapshots are
  connected.
