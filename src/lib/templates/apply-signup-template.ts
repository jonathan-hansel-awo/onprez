...
function createHeavenlyPamperPalaceSections(businessName: string): PageSection[] {
  return [
    {
    ...
    },
    {
      ...
      order: 5,
      isVisible: true,
      data: { title: 'Before your visit', items: [] },
    },
    {
...
function createRegentBarberSections(businessName: string): PageSection[] {
  return [
    {
      id: sectionId('navbar', 0),
      type: 'NAVBAR',
...
function createGenericTemplateSections(
  businessName: string,
  template: NonNullable<ReturnType<typeof getPresenceTemplate>>
): PageSection[] {
  return [
    {
      id: sectionId('navbar', 0),
    ...
    },
    {
      ...
      order: 5,
      isVisible: true,
      data: { title: 'Frequently Asked Questions', items: [] },
    },
    {
      id: sectionId('contact', 6),
      type: 'CONTACT',
...
export function createSignupPresencePageContent(
  ...
  if (!template) {
    return { sections: createDefaultPresencePageContent(businessName, category) }
  }

  const sections =
    template.slug === 'heavenly-pamper-palace'
      ? createHeavenlyPamperPalaceSections(businessName)
      : template.slug === 'regent-barber'
        ? createRegentBarberSections(businessName)
        : createGenericTemplateSections(businessName, template)
